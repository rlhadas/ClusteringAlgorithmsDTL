...
def k_means_step(graph, reps):
    ...
    print "%f %f %f %f" % cluster_quality(graph, [r[2] for r in res])
...
def k_means(graph, steps, k, seed=0, reps=None):
    ...
    if reps == None:
        ...
        k = len(reps)
    print "%f %f %f %f" % cluster_quality(graph, reps)
    print >> sys.stderr, '==========================='
...
def cluster_quality(graph, reps):
    ...
    average_ds = [float(sum(d[i] * i for i in d)) / sum(d.values()) for d in ds]

    k = len(reps)
    D = min([len(x ^ y) for x,y in itertools.combinations(reps, 2)] or [0]) / (2 * max(average_ds))
    DB = 1.0/k * sum( max( [(average_ds[i] + average_ds[j]) / len(reps[i] ^ reps[j]) for j in range(k) if i != j] or [0] ) for i in range(k) )

    return mean_cluster_radius, maximum_cluster_radius, D, DB
